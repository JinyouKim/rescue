...
class SoundManager():
    ...
    def startRecord(self):
        self.inputStream = sd.InputStream(channels=self.channels, samplerate=self.samples, device=self.inputDeviceName, dtype='int16', blocksize=160, callback=self.callbackInput, latency='high')
    ...
    def startPlay(self):
        self.outputStream = sd.RawOutputStream(channels=self.channels, samplerate=self.samples, device=self.outputDeviceName, dtype='int16', blocksize=160, callback=self.callbackOutput)
